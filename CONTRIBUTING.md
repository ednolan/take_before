...
configure beman.take_before to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_TAKE_BEFORE`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_TAKE_BEFORE" -C 2
```
...
### `BEMAN_TAKE_BEFORE_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.take_before` can use
`find_package(beman.take_before)` to locate the library.
